...
function money(value: { toString(): string } | null | undefined, currency: string) {
  return value
...
export default async function DealerQuoteDetailPage({
  params,
  searchParams,
}: PageProps<"/bayi/teklifler/[id]">) {
  const [{ id }, query, { company }] = await Promise.all([
    params,
    searchParams,
    requireDealerContext("/bayi/teklifler"),
  ]);
  ...
  const offerCurrency = quote.activeOfferRevision?.currency ?? quote.currency;
    ...
    <div className="grid gap-6">
      {query.created === "1" ? (
        <section className="flex gap-3 rounded-lg border border-emerald-200 bg-emerald-50 p-4 text-emerald-900">
          ...
          <div>
            <h2 className="font-semibold">Teklif talebiniz alındı</h2>
            <p className="mt-1 text-sm">
              Referans numaranız: <strong>{quote.quoteNumber}</strong>
            </p>
          </div>
        </section>
      ) : null}
        ...
        <div>
          ...
          <div className="mt-3 flex items-center gap-3">
            ...
            <span className="text-xs text-slate-500">
              {formatPortalDate(quote.createdAt)}
            </span>
          </div>
        </div>
        <Link
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_320px] lg:items-start">
        <section className="overflow-hidden rounded-lg border border-slate-200 bg-white shadow-sm">
          <div className="border-b border-slate-200 px-5 py-4">
            <h3 className="font-semibold">Talep kalemleri</h3>
          </div>
          <div className="divide-y divide-slate-200">
            {quote.items.map((item) => {
              const offerPrice = offerPrices.get(item.id);
              const currency = offerPrice ? offerCurrency : quote.currency;
              return (
                <article
                  key={item.id}
                ...
                >
                  <div>
                    <p className="font-mono text-xs font-semibold text-teal-800">
                      {item.product?.code ?? "ÖZEL"}
                    </p>
                    <h4 className="mt-1 font-semibold">
                      {item.product?.name ?? "Özel üretim talebi"}
                    </h4>
                    <p className="mt-2 text-xs text-slate-500">
                      {item.quantity} adet · {item.glassType ?? "Cam tipi teyit edilecek"} · {item.dimensions ?? "Ölçü teyit edilecek"}
                    </p>
                    {item.notes ? (
                  ...
                  <div className="sm:text-right">
                    <p className="text-sm font-semibold">
                      {money(offerPrice?.lineTotal ?? item.lineTotal, currency)}
                    </p>
                    <p className="mt-1 text-xs text-slate-500">
                      Birim: {money(offerPrice?.unitPrice ?? item.unitPrice, currency)}
                    </p>
                  </div>
                </article>
              );
        ...
        <aside className="grid gap-4 rounded-lg border border-slate-200 bg-white p-5 shadow-sm">
          <div>
            <p className="text-xs font-semibold uppercase text-slate-500">İletişim</p>
            <p className="mt-2 text-sm font-semibold">{quote.requesterName}</p>
            <p className="mt-1 text-sm text-slate-600">{quote.requesterEmail}</p>
            {quote.requesterPhone ? <p className="mt-1 text-sm text-slate-600">{quote.requesterPhone}</p> : null}
          </div>
          <div className="border-t border-slate-200 pt-4">
            <p className="text-xs font-semibold uppercase text-slate-500">İstenen teslim</p>
            <p className="mt-2 text-sm">{formatPortalDate(quote.desiredDeliveryDate)}</p>
          </div>
          <div className="border-t border-slate-200 pt-4">
            <p className="text-xs font-semibold uppercase text-slate-500">
              {quote.activeOfferRevision
                ? `Teklif toplamı · Revizyon ${quote.activeOfferRevision.revisionNumber}`
                : "Katalog tahmini"}
            </p>
            <p className="mt-2 text-lg font-semibold">
              {money(
                quote.activeOfferRevision?.subtotal ?? quote.estimatedSubtotal,
                quote.activeOfferRevision ? offerCurrency : quote.currency,
              )}
            </p>
            {!quote.activeOfferRevision && quote.hasUnpricedItems ? (
              <p className="mt-1 text-xs text-amber-700">
                Bazı kalemler satış ekibi tarafından fiyatlandırılacak.
              </p>
            ) : null}
          </div>
          {quote.notes ? (
            <div className="border-t border-slate-200 pt-4">
              <p className="text-xs font-semibold uppercase text-slate-500">Talep notu</p>
