    ...
    <header className="border-b border-slate-200 bg-white">
      <div className="border-b border-slate-100 bg-slate-950 text-slate-300">
        <div className="mx-auto flex min-h-8 max-w-[1440px] items-center justify-between px-4 text-xs md:px-6">
          ...
          <span className="hidden sm:inline">Satış desteği · Türkiye geneli B2B hizmet</span>
        </div>
      </div>

      <div className="mx-auto flex max-w-[1440px] flex-wrap items-center gap-3 px-4 py-4 md:flex-nowrap md:px-6">
        <Link href="/" className="flex shrink-0 items-center gap-3" aria-label="EkolGlass ana sayfa">
          <span className="flex h-11 w-11 items-center justify-center rounded-md bg-teal-800 text-white"><Factory size={22} aria-hidden="true" /></span>
          <span><strong className="block text-lg leading-5 text-slate-950">EkolGlass</strong><span className="text-xs text-slate-500">Camın Ekolü</span></span>
        </Link>

        <form action={productHref} className="order-3 flex h-11 w-full min-w-0 items-center rounded-md border border-slate-300 bg-slate-50 focus-within:border-teal-700 md:order-none md:mx-5 md:flex-1">
          <Search size={18} className="ml-3 shrink-0 text-slate-400" aria-hidden="true" />
          <input name="q" placeholder="Ürün kodu, OEM, marka, model veya ölçü ara" className="min-w-0 flex-1 bg-transparent px-3 text-sm outline-none" />
        ...
        {identity?.audience === "dealer" ? (
          ...
          <div className="ml-auto flex items-center gap-2">
            <Link href="/giris" className="inline-flex h-11 items-center gap-2 rounded-md border border-slate-300 px-3 text-sm font-semibold text-slate-800"><LogIn size={17} aria-hidden="true" />Bayi Girişi</Link>
            <Link href="/bayi-basvurusu" className="hidden h-11 items-center rounded-md bg-teal-800 px-4 text-sm font-semibold text-white sm:inline-flex">Bayi Ol</Link>
          </div>
        )}
      </div>
      ...
      <nav className="border-t border-slate-100" aria-label="Ana navigasyon">
        <div className="mx-auto flex min-h-11 max-w-[1440px] items-center gap-6 overflow-x-auto px-4 text-sm font-semibold text-slate-700 md:px-6">
          <Link href={productHref} className="whitespace-nowrap hover:text-teal-800">Ürünler</Link>
          <Link href="/#cozumler" className="whitespace-nowrap hover:text-teal-800">Çözümler</Link>
          ...
          {identity?.audience === "dealer" ? (
            <>
              ...
              <Link href="/bayi/teklifler" className="whitespace-nowrap text-teal-800">Tekliflerim</Link>
            </>
          ) : null}
        </div>
      </nav>
