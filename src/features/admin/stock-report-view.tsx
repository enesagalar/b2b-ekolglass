...
function queryHref(filters: ReturnType<typeof resolveStockReportFilters>, page: number) {
  const query = new URLSearchParams({ view: "stock", page: String(page), sort: filters.sort, productStatus: filters.productStatus });
...
export async function StockReportView({
  ...
  const metrics = [
    ...
    { label: "Stok sorunuz", value: report.metrics.askForAvailabilityCount, detail: "Manuel teyit gereken", icon: RefreshCw, tone: "bg-slate-100 text-slate-700" },
  ];
    ...
    <>
      ...
      <section className={`${panelClass} p-4`}>
        ...
        <div className="mt-3 flex items-center justify-between gap-3 border-t border-slate-100 pt-3 text-xs text-slate-500">
          ...
          <Link href="/admin/raporlar?view=stock" className="font-semibold text-teal-800">Filtreleri temizle</Link>
        </div>
      ...
      <section className={`${panelClass} overflow-hidden`}>
        ...
        {report.metrics.ledgerMismatchCount ? <p role="alert" className="mx-5 mt-4 rounded-md bg-red-50 p-3 text-sm font-semibold text-red-700">{report.metrics.ledgerMismatchCount} stok kaydında rezerve sayaç ile aktif rezervasyon defteri uyuşmuyor.</p> : null}
        {report.rows.length ? <div className="overflow-x-auto"><table className="w-full min-w-[1080px] text-left text-sm"><caption className="sr-only">Filtrelenmiş ürün ve depo stok kayıtları</caption><thead className="bg-slate-50 text-xs font-semibold uppercase text-slate-500"><tr><th scope="col" className="px-5 py-3">Ürün</th><th scope="col" className="px-5 py-3">Depo</th><th scope="col" className="px-5 py-3 text-right">Fiziksel</th><th scope="col" className="px-5 py-3 text-right">Rezerve</th><th scope="col" className="px-5 py-3 text-right">Kullanılabilir</th><th scope="col" className="px-5 py-3">Operasyon</th><th scope="col" className="px-5 py-3">Tanımlı durum</th><th scope="col" className="px-5 py-3">Görünürlük</th><th scope="col" className="px-5 py-3">Güncelleme</th></tr></thead><tbody className="divide-y divide-slate-100">{report.rows.map((row) => <tr key={row.id}><td className="px-5 py-4"><Link href={`/admin/urunler/${row.product.id}`} className="font-semibold text-teal-800">{row.product.code}</Link><p className="mt-1 max-w-xs text-xs text-slate-500">{row.product.name}</p></td><td className="px-5 py-4 font-semibold">{row.warehouseCode}</td><td className="px-5 py-4 text-right tabular-nums">{row.quantity}</td><td className="px-5 py-4 text-right tabular-nums">{row.reservedQuantity}</td><td className="px-5 py-4 text-right font-semibold tabular-nums">{row.availableQuantity}</td><td className="px-5 py-4"><span className={`inline-flex rounded px-2 py-1 text-xs font-semibold ${row.operationalClass === "AVAILABLE" ? "bg-emerald-50 text-emerald-800" : row.operationalClass === "LOW_AVAILABLE" ? "bg-amber-50 text-amber-800" : "bg-red-50 text-red-700"}`}>{row.operationalStatusLabel}</span></td><td className="px-5 py-4"><PortalStatus status={row.status} /></td><td className="px-5 py-4 text-slate-600">{row.visibilityLabel}</td><td className="px-5 py-4 text-slate-500">{formatPortalDate(row.updatedAt)}</td></tr>)}</tbody></table></div> : <div className="px-5 py-14 text-center"><p className="font-semibold text-slate-800">Filtrelerle eşleşen stok kaydı yok</p><Link href="/admin/raporlar?view=stock" className="mt-2 inline-block text-sm font-semibold text-teal-800">Filtreleri temizle</Link></div>}
        {report.pagination.pageCount > 1 ? <div className="flex flex-col gap-3 border-t border-slate-200 px-5 py-4 text-sm sm:flex-row sm:items-center sm:justify-between"><span>Sayfa {report.pagination.currentPage} / {report.pagination.pageCount}</span><div className="flex gap-2">{report.pagination.currentPage > 1 ? <Link href={queryHref(filters, report.pagination.currentPage - 1)} className="rounded-md border border-slate-300 px-3 py-2 font-semibold">Önceki</Link> : null}{report.pagination.currentPage < report.pagination.pageCount ? <Link href={queryHref(filters, report.pagination.currentPage + 1)} className="rounded-md border border-slate-300 px-3 py-2 font-semibold">Sonraki</Link> : null}</div></div> : null}
      </section>
    </>
  );
}
